 ...
 * 网易财经-宏观新闻
 */
@Component("netEasyMacroNewsPuller")
public class NetEasyMacroNewsPuller implements NewsPuller {
    private static final SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    private static final Logger logger = LoggerFactory.getLogger(NetEasyMacroNewsPuller.class);
    @Autowired
    NewsMapper newsMapper;
    @Value("${news.neteasy.macrourl}")
    private String url;
    @Autowired
    ...
    public void pullNews() {
        ...
        logger.info("开始拉取网易财经-宏观新闻！");
        // 1.获取首页
        Document html = null;
        try {
            html = getHtmlFromUrl(url, false);
        } catch (Exception e) {
            logger.error("==============拉取网易财经-宏观新闻失败: {}=============", url);
            e.printStackTrace();
            return;
        }
        Elements newsElements = html.select("div.item_top").select("div.item_top h2 a");
        ...
        if (newsType1Set.size() > 0) {
            newsType1Set.forEach(news -> {
                logger.info("开始抽取网易宏观新闻内容：{}", news.getUrl());
                ...
                try {
                    ...
                    String time = postInfoElement.text();
                    // 获取图片URL
                    ...
                    logger.info("抽取网易宏观新闻《{}》成功！", news.getTitle());
                } catch (Exception e) {
                    logger.error("网易宏观新闻抽取失败:{}", news.getUrl());
                    e.printStackTrace();
                }
            });
            logger.info("网易宏观新闻拉取完成！");
        }
